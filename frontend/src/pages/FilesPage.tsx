...
export function FilesPage() {
	...
	const handleDelete = async () => {
		...
		if (
			!window.confirm(
				`Remove ${selectedIds.size} file record(s) from database?`,
			)
		)
			return;
	...
	const handleConvert = async (converter: string) => {
		...
		for (const id of selectedIds) {
			try {
				...
				toast.error(`Convert failed for ${id}`, {
	...
	const handleVerify = async () => {
		...
		try {
			...
			if (problems > 0) {
				toast.error(
					`Verified ${res.total} files: ${res.ok} ok, ${res.mismatch} mismatch, ${res.missing} missing`,
				);
			} else {
				toast.success(
					`All ${res.ok} files verified OK${res.skipped > 0 ? ` (${res.skipped} skipped)` : ""}`,
				);
			}
		} catch (e) {
				...
				<CardTitle className="flex items-center justify-between">
					...
					<div className="flex items-center gap-2">
						...
						{badFileIds.length > 0 && (
							...
							>
								<RefreshCw
									className={`mr-2 h-4 w-4 ${resyncing ? "animate-spin" : ""}`}
								/>
								Re-sync {badFileIds.length} files
							</Button>
						)}
						{selectedIds.size > 0 &&
							activeConverters.map((c) => (
								<Button
									key={c.name}
									size="sm"
									variant="outline"
									onClick={() => handleConvert(c.name)}
									disabled={converting}
								>
