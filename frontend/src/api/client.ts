...
export const getAuthStatus = () =>
	request<{ authenticated: boolean; pending: boolean; auth_method: string }>(
		"/auth/status",
	);
...
export const startIncrementalSync = () =>
	request<{ status: string; mode: string }>("/sync/incremental", {
		method: "POST",
	});
...
export const previewSelect = (patterns: string[]) =>
	request<{ total: number; matched: number; samples: string[] }>(
		"/sync/preview",
		{
			method: "POST",
...
export const getSyncStatus = () =>
	request<{ is_running: boolean; progress: import("./types").SyncProgress }>(
		"/sync/status",
	);
...
export const listFiles = (search = "") =>
	request<import("./types").SyncedFile[]>(
...
export const verifyFiles = () =>
	request<import("./types").VerifyResponse>("/files/verify", {
		method: "POST",
	});
...
export const downloadRevision = (
	...
	request<{ path: string; size: number }>(
		`/files/${fileId}/revisions/${revId}/download`,
		{
			method: "POST",
...
export const listDownloadedRevisions = (fileId: string) =>
	request<import("./types").DownloadedRevision[]>(
		`/files/${fileId}/revisions/downloaded`,
	);
...
export const updateTarget = (
	name: string,
	target: import("./types").DistTarget,
) =>
...
export const testTarget = (name: string) =>
	request<{ status: string }>(`/distribution/targets/${name}/test`, {
		method: "POST",
	});
export const distribute = (
	...
	request<{ file_id: string; status: string; path?: string; error?: string }[]>(
		"/distribute",
		{
			method: "POST",
			body: JSON.stringify({
...
// Drive Browse
export const browseDrive = (
	folderId?: string,
	source?: "my_drive" | "shared",
) => {
	const params = new URLSearchParams();
	if (folderId) params.set("folder_id", folderId);
	if (source) params.set("source", source);
	const qs = params.toString();
	return request<import("./types").DriveBrowseResult>(
		`/drive/browse${qs ? `?${qs}` : ""}`,
	);
};
...
export const convertFile = (fileID: string, converter: string) =>
	request<{ status: string; job_id: string }>(`/files/${fileID}/convert`, {
		method: "POST",
		body: JSON.stringify({ converter }),
	});
export const reconvertFile = (fileID: string, converter: string) =>
	request<{ status: string; job_id: string }>(`/files/${fileID}/reconvert`, {
		method: "POST",
		body: JSON.stringify({ converter }),
	});
