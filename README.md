...
│   ├── sync/               # 同期エンジン, 差分判定 (MD5), 選択同期 (select.go), 進捗追跡
│   ├── server/             # Chi v5 ルーター, ミドルウェア (CORS/Logger/Recovery), WebSocket Hub
│   ├── handler/            # REST API エンドポイント (auth, sync, files, verify, browse, drive_browse, distribution)
...
| `GET` | `/ws` | WebSocket（リアルタイム進捗・検証進捗） |
